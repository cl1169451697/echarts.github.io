    ...
    series: [
      {
        ...
        itemStyle: {
          ...
          color(params) {
            // params 柱子的数量
            console.log(params);
            return myColor[params.dataIndex];// 根据索引显示对于的颜色
            console.log(params);
          },
        },
        // 柱子间的距离
        ...
        label: {
          ...
          position: 'inside',
          // 文字的显示格式
          formatter: '{c}%', // {c} 自动的解析为 数据 data里面的数据
        },
      },
      {
        name: '框',
...
(function () {
  ...
  const option = {
    ...
    legend: {
      // 如果 series 对象有name 值 , 则legend可以不用
      // data: ['邮件营销', '联盟广告'],
      textStyle: {
        color: '#4c9bfd', // 图例颜色
      },
  ...
  $('.line h2').on('click', 'a', function () {
    // console.log($(this).index());// 获取当前索引值
    const obj = (yearData[$(this).index()]);// 根据索引获取假数据里对应的数组
    option.series[0].data = obj.data[0];// 然后赋值给series.data
    option.series[1].data = obj.data[1];
    myChart.setOption(option);// 最后重新渲染
  });
}());
...
(function () {
  ...
  const series = [];
  [['西安', XAData], ['西宁', XNData], ['银川', YCData]].forEach((item, i) => {
    series.push({
      name: `${item[0]} Top3`,
      type: 'lines',
      zlevel: 1,
